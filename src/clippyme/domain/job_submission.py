...
def write_publisher_mode(job_output_dir: str, publisher_mode: str) -> None:
    ...
    publishing for). Best-effort: a write failure is logged, never raised —
    the job submission itself must not fail over this.
    """
    ...
    try:
        ...
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            json.dump({"publisher_mode": mode}, f)
...
async def submit_job(*, jobs: dict, job_queue: asyncio.Queue, job_id: str,
                     ...
                     publisher_mode: str = "manual_queue") -> None:
    ...
    write_publisher_mode(job_output_dir, publisher_mode)
    try:
