...
function ResultCard({
    ...
    // Cover/poster frame: the pipeline writes `{clip}_cover.jpg` next to every
    // clip (select_cover_frame) and the /videos mount serves it. Deriving the
    // poster from the clean clip.video_url means each card shows a real
    // thumbnail instead of a black <video> until the user hits play. Cache-bust
    // with reframeBust so a re-reframed clip's regenerated cover is re-fetched.
    const coverUrl = React.useMemo(() => {
        const base = clip.video_url || '';
        const jpg = base.replace(/\.mp4$/i, '_cover.jpg');
        if (!base || jpg === base) return undefined;
        const bust = clipState.reframeBust ? `?v=${clipState.reframeBust}` : '';
        return getApiUrl(jpg + bust);
    }, [clip.video_url, clipState.reframeBust]);
    ...
    return (
        ...
        >
            ...
            <div className="relative w-full aspect-[9/16] bg-black overflow-hidden">
                ...
                {/* Reframing overlay — the clip file is being regenerated by
                    /api/reframe; dim the stale frame + show progress so the
                    user knows the preview is mid-update (not frozen/broken). */}
                {isReframing && (
                    <div className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-2 bg-black/55 backdrop-blur-[1px]">
                        <Loader2 size={26} className="animate-spin text-[oklch(82%_0.16_68)]" strokeWidth={2} />
                        <span className="type-mono text-[10px] uppercase tracking-[0.16em] text-zinc-200">
                            Reframing…
                        </span>
                    </div>
                )}
                        ...
                        <div className="space-y-2 mt-2 animate-fade-in">
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-0.5">
                                    ...
                                    >
                                        {copiedField === 'title' ? <Check size={11} className="text-[oklch(78%_0.17_145)]" /> : <Copy size={11} />}
                                    </button>
                                </div>
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-0.5">
                                    ...
                                    <button
                                        ...
                                        className="p-1 text-zinc-500 hover:text-white rounded transition-colors"
                                    >
                                        {copiedField === 'caption' ? <Check size={11} className="text-[oklch(78%_0.17_145)]" /> : <Copy size={11} />}
                                    </button>
                                </div>
